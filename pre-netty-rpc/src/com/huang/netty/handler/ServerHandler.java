...
import com.huang.netty.medium.Media;
...
public class ServerHandler extends ChannelInboundHandlerAdapter {//Netty 提供的默认实现类，用于处理入站事件（如接收数据），可以通过重写方法来实现自定义行为。
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception{//当服务器接收到客户端发送的消息时会触发。
        ...
        System.out.println("解析后的请求: " + request);
        ...
        Media media = Media.newInstance();//这一行代码是创建或获取 Media 类的唯一实例。
        Response result = media.process(request);//这一行代码是调用 Media 实例的 process 方法，用于处理请求 request 并返回结果 result。
        ...
        System.out.println("返回的响应: " + result);
        ctx.channel().writeAndFlush(JSONObject.toJSONString(result));//将 Response 对象序列化为 JSON 字符串，并通过 Netty 的通道（Channel）发送回客户端。
